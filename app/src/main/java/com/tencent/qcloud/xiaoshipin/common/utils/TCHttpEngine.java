...
//            = MediaType.parse("application/json; charset=utf-8");
    ...
    public static String getErrorMessage(Object error) {
        if (error instanceof TimeoutError) {
            return TCApplication.getApplication().getResources().getString(R.string.tc_http_engine_get_error_message_connect_server_failed);
        } else if (isServerProblem(error)) {
            return handleServerError(error);
        } else if (isNetworkProblem(error)) {
            return TCApplication.getApplication().getResources().getString(R.string.tc_http_engine_get_error_message_no_network_connection);
        }
        return TCApplication.getApplication().getResources().getString(R.string.tc_http_engine_get_error_message_network_anomaly);
    }
